...
#
# Every variable below (except `project_id`) exists with the same name, type and
# meaning in stacks/aws. Switching clouds means changing `region` and pointing
...
# --- GCP-only additions ------------------------------------------------------
# The one variable AWS does not need. Everything above is shared verbatim.
variable "project_id" {
